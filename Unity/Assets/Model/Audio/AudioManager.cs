...
{
    // TODO :: consider making the Sound Effect multifaceted / multifunctional   考虑到音效的多方面和多功能性
    // meaning you can add a sound effect as a monobehaviour to do other functions 意味着你可以添加音效作为单一行为来完成其他功能，
    // like allow a sound effect play a sound or respond to events  比如让播放 音效 或 响应事件都可以进行
...
public class AudioManager : MonoBehaviour
{
    ...
    [Tooltip("混音器 The target group for the background music to route its their signals. If none is to be used, then leave unassigned or blank")]
    ...
    [Tooltip("混音器 The target group for the sound effects to route its their signals. If none is to be used, then leave unassigned or blank")]
    ...
    private void ManageSoundEffects()
    {
        ...
        for (int i = sfxPool.Count - 1; i >= 0; i--)
        {
            ...
            //To prevent large numbers of sound from playing simultaneously, the prefabricated body is not eliminated. At the same time, the sound effects that are not played will not be executed directly.
            if (!sfxPool[i].Source.isPlaying)
            {
                if (sfxPool[i].Callback != null)
                {
                    sfxPool[i].Callback.Invoke();
    ...
    {
        return PlayerPrefs.HasKey(PlayerPrefsKeys.BgMusicVolKey) ? PlayerPrefsMgr.mInstance.GetFloat(PlayerPrefsKeys.BgMusicVolKey) : _musicVolume;
    }
    ...
    {
        return PlayerPrefs.HasKey(PlayerPrefsKeys.SoundFxVolKey) ? PlayerPrefsMgr.mInstance.GetFloat(PlayerPrefsKeys.SoundFxVolKey) : _soundFxVolume;
    }
    ...
    {
        return PlayerPrefs.HasKey(PlayerPrefsKeys.BgMusicMuteKey) ? ToBool(PlayerPrefsMgr.mInstance.GetInt(PlayerPrefsKeys.BgMusicMuteKey)) : _musicOn;
    }
    ...
    {
        return PlayerPrefs.HasKey(PlayerPrefsKeys.SoundFxMuteKey) ? ToBool(PlayerPrefsMgr.mInstance.GetInt(PlayerPrefsKeys.SoundFxMuteKey)) : _soundFxOn;
    }
    ...
    {
        PlayerPrefsMgr.mInstance.SetInt(PlayerPrefsKeys.BgMusicMuteKey, _musicOn ? 1 : 0);
        PlayerPrefsMgr.mInstance.SetFloat(PlayerPrefsKeys.BgMusicVolKey, _musicVolume);
        PlayerPrefs.Save();
    }
    ...
    {
        PlayerPrefsMgr.mInstance.SetInt(PlayerPrefsKeys.SoundFxMuteKey, _soundFxOn ? 1 : 0);
        PlayerPrefsMgr.mInstance.SetFloat(PlayerPrefsKeys.SoundFxVolKey, _soundFxVolume);
        PlayerPrefs.Save();
    }
    ...
    {
        PlayerPrefs.DeleteKey(PlayerPrefsKeys.BgMusicVolKey);
        PlayerPrefs.DeleteKey(PlayerPrefsKeys.SoundFxVolKey);
        PlayerPrefs.DeleteKey(PlayerPrefsKeys.BgMusicMuteKey);
        PlayerPrefs.DeleteKey(PlayerPrefsKeys.SoundFxMuteKey);
        PlayerPrefs.Save();
    }
